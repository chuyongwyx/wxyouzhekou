...
Component({
  ...
  properties: {
    ...
    per: { //百分比 通过此值转换成step
      ...
      observer: function (newVal, oldVal){
        ...
        const per = newVal ; //圆形进度
        const r = Number(_this.data.r); //圆形半径
        _this.setData({
          step: (2 * Number(newVal)) / 100,
          txt: newVal
        });
  ...
  methods: {
    ...
    drawCircleBg: function (el, r, w) {
      ...
      ctx.setStrokeStyle('#f3f3f3'); // 设置圆环的颜色
 ...
 */
    drawCircle: function (el, r, w, step) {
      ...
      // 设置渐变
      var gradient = context.createLinearGradient(2*r,r,r,2*r);
      gradient.addColorStop("0", "#ff7715")
      gradient.addColorStop("0.5", "#ffbc1e")
      gradient.addColorStop("1", "#ff7715")
